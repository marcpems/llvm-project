...
// This is the *unmodified* body of upstream llvm/utils/FileCheck/FileCheck.cpp's
// main(), just renamed and exported so it can be called in-process from a
// long-lived host (instead of only ever running as a fresh process's entry
// point). Deliberately not otherwise touched, so it stays byte-for-byte
// behaviorally identical to real FileCheck.exe -- the entire point of this
// approach vs. the previous hand-rolled argv-parsing shim is to stop
// reimplementing the CLI driver's behavior and just call it directly.
extern "C" __declspec(dllexport) int FileCheckMain(int argc, char **argv) {
  // Enable use of ANSI color codes because FileCheck is using them to
  // highlight text.
  llvm::sys::Process::UseANSIEscapeCodes(true);

  // NOTE: deliberately *not* constructing InitLLVM here (unlike upstream's
  // main()). InitLLVM's Windows-specific constructor calls
  // windows::GetCommandLineArguments(), which re-fetches the *real OS
  // process* command line via GetCommandLineW() and overwrites our argc/argv
  // parameters with it, discarding whatever was actually passed in. That is
  // harmless for a normal freshly-spawned "FileCheck.exe ..." process (the
  // real OS command line and the passed argv happen to be identical), but it
  // is fatal for this function's real purpose: being called many times, with
  // different per-call argv, from a single long-lived host process (e.g. a
  // Python worker via ctypes) -- every call would silently ignore its own
  // argv and instead re-parse the *host's* command line (e.g. lit's own
  // "python -m lit ..."). Skipping InitLLVM only loses its signal-handler/
  // stack-trace niceties, which are not needed for embedded use.
  //
  // Reset cl::opt global state left over from any previous call in this
  // process -- required for reentrancy, since cl::list options (like
  // --check-prefix/-D) otherwise accumulate values across calls instead of
  // reflecting only the current argv.
  cl::ResetAllOptionOccurrences();
  // Pass a real Errs stream (instead of nullptr) so a parse error returns
  // false instead of calling exit(1), which would tear down the entire host
  // process instead of just failing this one call.
